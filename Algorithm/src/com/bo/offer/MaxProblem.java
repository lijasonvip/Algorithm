...
public class MaxProblem {
	...
	/**
	 * 分治法求最大子序列和 最大序列和可能出现三个地方 左半部分，右半部分，跨数据中部占左右两半部分
	 */
	...
	public static int find_max_crossing_subarray(int A[], int low, int mid, int hight)
	{
	    int left_sum = -1000, right_sum = -10000;//一个比较小的负数即可
	    int sum = 0;
	    int i, j;
	    for(i=mid; i>= low; i--)
	    {
	...
	/**
	 * 重复次数最的数字 http://www.acmerblog.com/maximum-repeating-number-6035.html
	 * 遍历arr[]中的元素，做如下操作 arr[arr[i]%k] += k . 之所以增加k是 因为可以恢复修改过的arr[i]，它最初的数据即为
	 * arr[i] % k , 同时 arr[i] / k 就代表 数字 i 出现的次数。此方法的前提是要保证计算不会溢出
	 */
	public static int maxRepeating(int[] arr, int k) {
		...
		for (int i = 0; i < arr.length; i++) {
			arr[arr[i] % k] += k;
			// arr[arr[i]%k] / k 代表 arr[i]%k 出现的次数,
			// 由于k是固定的，这里可以直接用arr[arr[i]%k]比较
